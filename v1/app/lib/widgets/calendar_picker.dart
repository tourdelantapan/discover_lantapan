...
class DateSetting {
  DateSetting(this.color, this.position);
...
class _CustomCalendarViewState extends State<CustomCalendarView> {
  ...
  DateTime? startDate;
  DateTime? endDate;
  Map<DateTime, DateSetting> bookedDates = {};
                ...
                children: <Widget>[
                  ...
                  InkWell(
                    ...
                    onTap: () {
                      ...
                      if (widget.selectionMode == "ROOM" &&
                          bookedDates.containsKey(date) &&
                          bookedDates[date]!.position == "FIRST" &&
                          startDate == null) {
                        dialogBuilder(context,
                            title: "Notice",
                            description: "Selected date is for checkout only");
                        return;
                      }

                      if (widget.selectionMode == "ROOM" &&
                          bookedDates.containsKey(date) &&
                          bookedDates[date]!.position == "MIDDLE") {
                        return;
                      }

                      if (startDate != null && !widget.singleDaySelection) {
                        DateTime dateA;
                        DateTime dateB;
                        ...
                        int flag = 0;
                        Duration diff = dateB.difference(dateA);
                        int days = diff.inDays;

                        for (int i = 1; i < days; i++) {
                          Duration add = Duration(days: i);
                          if (bookedDates.containsKey(DateTime(
                              dateA.add(add).year,
                              dateA.add(add).month,
                              dateA.add(add).day))) {
                            flag = 1;
                        ...
                        if (flag == 1) {
                          dialogBuilder(context,
                              title: "Notice",
                              description:
                                  "You selected blocked/booked dates.");
                          return;
                        }
                      }
