...
var Transactionwatch = {
  init: function (db, _collection, options) {
    ...
    if (!(db && types.includes(dbType)))
      throw new Error(
        "Please specify a mongoose instance or a connection string"
      );
...
function checkInitStatus() {
  if (!Roller) {
    throw new Error(
      "Transactionwatch has not been initialized. Call Transactionwatch.init"
    );
  }
}
